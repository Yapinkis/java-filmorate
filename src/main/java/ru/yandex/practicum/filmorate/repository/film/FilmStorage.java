...
public class FilmStorage implements FilmRepository {
    ...
    public void removeLike(Film film, User userLike) {
        ...
        //Насколько я понимаю, в repository мы обычно исключения не обрабатываем. Оставлять код таким образом и
        // ловить исключения в слое сервиса или всё-таки делать это здесь?Т.к. легко может быть попытка проверить
        // лайки у фильма, которому их ещё не поставили
    }
